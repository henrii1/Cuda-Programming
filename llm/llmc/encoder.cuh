...
/* Cuda Kernels*/
// floatX is user specified
__global__ void encoder_forward_kernel3(floatX* out,
                                        const int* inp, const floatX* wte, const floatX* wpe,
                                        int B, int T, int C){
    int idx = (blockIdx.x * BlockDim.x + threadIdx.x) * x128::size;
    int N = B * T * C;  //total number of elements
    if (idx>=N) return;

    int bt = idx / C;  // idx is now (batch, time, channel)
    int b = bt / T; //batch index
    int t = bt % T; //time index
    int c = idx % C; //channel index

    int ix = inp[b * T + t]; //token index

    floatX* out_btc = out + b * T * C + t * C + c;   // seek to the right position
    const floatX* wte_ix = wte + ix * C + c; // seek to the right token embedding
    const floatX* wpe_tc = wpe + t * C + c; // seek to the right position embedding
...
// NOTE: int4 contains 4 integers. one int is 32 bits signed

template <int BLOCK_SIZE=265>
__global__ void wte_backward_kernel(floatX* dwte, const int4* bucket_info, const int* workload_indices,
                                    const floatX* dout, const int* inp, unsigned int seed, int B, int T, int C){
    ...
    int lane_id = threadIdx.x % WARP_SIZE; // warp lane
    int c_per_warp = WARP_SIZE * x128::size; // number of channels per warp

    int bucket_start_idx = bucket_info[bucket].x;
    int bucket_size = bucket_info[bucket].y;
    int bucket_ix = bucket_info[bucket].x;
